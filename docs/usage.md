...
excludes total Hb. If the LABNIRS file only contains Hb data, `--type` is
unnecessary.

## Command-Line Options

All of the following calls will print the usage instructions:
...
Text file exported by the LabNIRS software. LABNIRS allows to select whether to
include _voltage_ and/or _raw_ data, and obviously, if one data type is not
included, the relevant `--drop` and `--type` switches may not work as intended.
It is possible to run into an error for not having any data when dropping
...
Probe layout files specify the 3D positions of sources and detectors. The expected
format is a .sfp file, which is a tab-separated text file with columns:
